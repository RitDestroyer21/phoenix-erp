...
export function FacultyProfileModal({ faculty, onClose }: Props) {
  return (
        ...
        <div className="pt-16 px-8 pb-8 space-y-8">
          ...
          <div>
            ...
            <p className="text-red-600 font-bold text-sm tracking-widest uppercase">
              {"Faculty Member"} — {faculty.department_name}
            </p>
            <div className="flex items-center gap-2 mt-2 opacity-50 text-xs font-mono">
              <Hash size={12} /> {faculty.faculty_id}
            </div>
          </div>

          <div className="grid md:grid-cols-2 gap-8">
            
            {/* Left Column: Employment & Personal */}
            <div className="space-y-6">
              <section>
                <h4 className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-3">Employment Details</h4>
                <div className="space-y-3">
                  <InfoRow icon={<GraduationCap size={14}/>} label="Role" value={"Instructor"} />
                </div>
              </section>
              ...
              <section>
                ...
                <div className="space-y-3">
                  <InfoRow icon={<Calendar size={14}/>} label="DOB" value={faculty.user_basic_details?.user_basic_details_dob || "N/A"} />
                  <InfoRow icon={<User size={14}/>} label="Gender" value={faculty.user_basic_details?.user_basic_details_gender?.toLowerCase() || "N/A"} />
                </div>
              </section>
            </div>

            {/* Right Column: Contact info mapping */}
            <div className="space-y-6">
              <section className="bg-zinc-50 dark:bg-zinc-900/50 p-4 rounded-2xl border border-zinc-100 dark:border-zinc-800">
                ...
                <div className="space-y-4">
                  <ContactItem 
                    ...
                    value={faculty.user_basic_details?.user_contact_details?.contact_mail1 || "N/A"} 
                  />
                  <ContactItem 
                    icon={<Phone className="text-red-600" size={16}/>} 
                    label="Primary Phone" 
                    value={faculty.user_basic_details?.user_contact_details?.contact_phone1?.toString() || "N/A"} 
                  />
                  
                  {/* Optional Personal Mail Fallback */}
                  {faculty.user_basic_details?.user_contact_details?.contact_mail2 && (
                    <ContactItem 
                      icon={<Mail size={16}/>} 
                      label="Personal Email" 
                      value={faculty.user_basic_details.user_contact_details.contact_mail2} 
                    />
                  )}

                  {/* Optional Alternate Phone Fallback */}
                  {faculty.user_basic_details?.user_contact_details?.contact_phone2 && (
                    <ContactItem 
                      icon={<Phone size={16}/>} 
                      label="Alternate Phone" 
                      value={faculty.user_basic_details.user_contact_details.contact_phone2.toString()} 
                    />
                  )}
                </div>
              </section>
